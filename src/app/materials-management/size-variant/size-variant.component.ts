...
export class SizeVariantComponent implements AfterViewInit {
  displayedColumns = ['SizeDescription', 'CreateDate', 'CreatedBy', 'LastUpdate', 'UpdatedBy'];
  dataSource: DataTable<SizeVariant>;
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(ModalDirective) modal: ModalDirective;

  constructor(private sizeVariantService: SizeVariantService,
              private router: Router,
              private route: ActivatedRoute,
              private modalService: ModalService) {
    this.dataSource =
      new DataTable<SizeVariant>(this.displayedColumns,
        this.sizeVariantService,
        this.router,
        this.route);
  }
    ...
    switch (operation) {
      ...
      case Operations.View:
        if (this.dataSource.row)
          this.router?.navigate([this.dataSource.row?.sizeId], {
            relativeTo: this.route,
            queryParamsHandling: 'preserve'
          });
        break;
      case Operations.Edit:
        if (this.dataSource.row)
          this.router?.navigate([this.dataSource.row?.sizeId, 'edit'], {
            relativeTo: this.route,
            queryParamsHandling: 'preserve'
          });
        break;
        ...
        if (this.dataSource.row) {
          this.modalService.show(this.modal.viewContainerRef, {
            successCallback: () => {
              ...
              this.sizeVariantService.delete(this.dataSource.row?.sizeId)
                .subscribe({
                  next: _ => {
