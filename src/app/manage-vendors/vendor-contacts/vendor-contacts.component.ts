...
export class VendorContactsComponent implements AfterViewInit {
  displayedColumns = ['CompanyName', 'Name', 'EmailAddress', 'EmailDistribution', 'PhoneNumber', 'MobileNumber',
    'CreateDate', 'CreatedBy', 'LastUpdate', 'UpdatedBy'];
  dataSource: DataTable<VendorContact>;
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(ModalDirective) modal: ModalDirective;

  constructor(private contactsService: VendorContactsService,
              private router: Router,
              private route: ActivatedRoute,
              private modalService: ModalService) {
    this.dataSource =
      new DataTable<VendorContact>(this.displayedColumns,
    ...
    switch (operation) {
      ...
      case Operations.View:
        if (this.dataSource.row)
          this.router?.navigate([this.dataSource.row?.contactId], {
            relativeTo: this.route,
            queryParamsHandling: 'preserve'
          });
        break;
      case Operations.Edit:
        if (this.dataSource.row)
          this.router?.navigate([this.dataSource.row?.contactId, 'edit'], {
            relativeTo: this.route,
            queryParamsHandling: 'preserve'
          });
        break;
        ...
        if (this.dataSource.row) {
          this.modalService.show(this.modal.viewContainerRef, {
            successCallback: () => {
              ...
              this.contactsService.delete(this.dataSource.row?.contactId)
                .subscribe({
                  next: _ => {
