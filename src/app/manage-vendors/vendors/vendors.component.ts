...
export class VendorsComponent implements AfterViewInit {
  displayedColumns = ['CompanyName', 'Email', 'PhoneNumber', 'StateName', 'CreateDate', 'CreatedBy', 'LastUpdate', 'UpdatedBy'];
  dataSource: DataTable<Vendor>;
  ...
  constructor(private vendorsService: VendorsService,
    ...
    this.dataSource =
      new DataTable<Vendor>(this.displayedColumns,
        this.vendorsService,
        this.router,
        this.route);
  }
    ...
    switch (operation) {
      ...
      case Operations.View:
        if (this.dataSource.row)
          this.router?.navigate([this.dataSource.row?.vendorId], {
            relativeTo: this.route,
            queryParamsHandling: 'preserve'
          });
        break;
      case Operations.Edit:
        if (this.dataSource.row)
          this.router?.navigate([this.dataSource.row?.vendorId, 'edit'], {
            relativeTo: this.route,
            queryParamsHandling: 'preserve'
          });
        break;
        ...
        if (this.dataSource.row) {
          this.modalService.show(this.modal.viewContainerRef, {
            successCallback: () => {
              ...
              this.vendorsService.delete(this.dataSource.row?.vendorId)
                .subscribe({
                  next: _ => {
